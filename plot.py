...
def _plot_param_table(parameters, web=False, colors=None):
    ...
    kwargs = {'color':colors['text'], 'fontsize':10, 'clip_on':False, 'transform':trans}

    pylab.text(start_x + 0.175, start_y, "Parameters", ha='center', fontweight='bold', **kwargs)

    spacer = Line2D([start_x, start_x + 0.361], [line_y - line_space * 0.48] * 2, color=colors['text'], linestyle='-', transform=trans, clip_on=False)
    ...
    if not web:
        ...
        pylab.text(start_x + 0.305,  line_y + 0.009, "2   -2", fontweight='bold', color=colors['text'], fontsize=6, clip_on=False, transform=trans)
...
def _plot_data(data, parameters, colors=None):
    storm_dir, storm_spd = parameters['storm_motion']
    bl_dir, bl_spd = parameters['bunkers_left']
    br_dir, br_spd = parameters['bunkers_right']
    mn_dir, mn_spd = parameters['mean_wind']
    ...
    pylab.plot(mkr_u, mkr_v, 'o', color=colors['text'], ms=10)
    for um, vm, zm in zip(mkr_u, mkr_v, mkr_z):
        if not np.isnan(um):
            pylab.text(um, vm - 0.1, str(zm), va='center', ha='center', color=colors['marker_text'], size=6.5, fontweight='bold')
    ...
    if not (np.isnan(bl_u) or np.isnan(bl_v)):
        pylab.plot(bl_u, bl_v, 'o', color=colors['bunkers_left'], markersize=5, mfc='none')
        pylab.text(bl_u + 0.5, bl_v - 0.5, "LM", ha='left', va='top', color=colors['bunkers_left'], fontsize=10)

    if not (np.isnan(br_u) or np.isnan(br_v)):
        pylab.plot(br_u, br_v, 'o', color=colors['bunkers_right'], markersize=5, mfc='none')
        pylab.text(br_u + 0.5, br_v - 0.5, "RM", ha='left', va='top', color=colors['bunkers_right'], fontsize=10)

    if not (np.isnan(mn_u) or np.isnan(mn_v)):
        pylab.plot(mn_u, mn_v, 's', color=colors['mean_wind'], markersize=5, mfc='none')
        pylab.text(mn_u + 0.6, mn_v - 0.6, "MEAN", ha='left', va='top', color=colors['mean_wind'], fontsize=10)
    ...
    if not (np.isnan(storm_u) or np.isnan(storm_v)) and not (smv_is_brm or smv_is_blm or smv_is_mnw):
        pylab.plot(storm_u, storm_v, '+', color=colors['storm_motion'], markersize=6)
        pylab.text(storm_u + 0.5, storm_v - 0.5, "SM", ha='left', va='top', color=colors['storm_motion'], fontsize=10)


def _plot_background(min_u, max_u, min_v, max_v, colors=None):
    ...
    pylab.axvline(x=0, linestyle='-', color=colors['background_line'])
    pylab.axhline(y=0, linestyle='-', color=colors['background_line'])

    for irng in range(10, max_ring, 10):
        ring = Circle((0., 0.), irng, linestyle='dashed', fc='none', ec=colors['background_line'])
        pylab.gca().add_patch(ring)

        if irng <= max_u - 10:
            rng_str = "%d kts" % irng if max_u - 20 < irng <= max_u - 10 else "%d" % irng

            pylab.text(irng + 0.5, -0.5, rng_str, ha='left', va='top', fontsize=9, color=colors['background_line'], clip_on=True, clip_box=pylab.gca().get_clip_box())


def plot_hodograph(data, parameters, fname=None, web=False, fixed=False, archive=False, dark=False):
    ...
    _plot_param_table(parameters, web=web, colors=colors)
    ...
    if not archive:
        pylab.title(img_title, color=colors['text'])
        pylab.text(0., -0.01, age_str, transform=pylab.gca().transAxes, ha='left', va='top', fontsize=9, color=colors['text'])
    else:
        pylab.title(img_title, color=colors['text'])
